...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("issues authenticated one-time pairing credentials for additional clients", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(bootstrapResult.body.scopes, AuthStandardClientScopes);
  ...
  it.effect("issues administrative-scope pairing credentials for sessions that request them", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const bootstrapResult = yield* bootstrapBrowserSession(body.credential);
      assert.equal(bootstrapResult.response.status, 200);
      assert.deepEqual(bootstrapResult.body.scopes, AuthAdministrativeScopes);
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
