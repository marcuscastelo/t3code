...
const decodeResumeCursor = Schema.decodeUnknownEffect(
  Schema.fromJsonString(Schema.Struct({ threadId: Schema.optional(Schema.Unknown) })),
);
...
const makeCodexSessionImporter = Effect.gen(function* () {
  ...
  const importThread: CodexSessionImporterShape["importThread"] = ({ threadId }) =>
    Effect.gen(function* () {
      ...
      const resumeCursor =
        row.resumeCursorJson === null
          ? {}
          : yield* decodeResumeCursor(row.resumeCursorJson).pipe(Effect.orElseSucceed(() => ({})));
