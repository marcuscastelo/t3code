...
export class WsTransport {
  ...
  private readonly transportCloseReconnectTimers = new Set<Fiber.Fiber<void, never>>();
  ...
  private createSession(): TransportSession {
    ...
    const protocolLayer = protocolFactory(this.url, {
      ...
      onClose: (details, context) => {
        lifecycleHandlers?.onClose?.(details, context);
        ...
        const timer = Effect.runFork(
          Effect.sleep(Duration.millis(TRANSPORT_CLOSE_RECONNECT_FALLBACK_DELAY_MS)).pipe(
            Effect.andThen(
              Effect.sync(() => {
